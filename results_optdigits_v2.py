...
df1 = pd.read_csv('optdigit.csv', sep=',', header=0)
...
def union_accuracy_median(a,b, c, d, e, X_test, y_test):
    unionnn = []
    for i in range(len(X_test)):
        ...
        for j in range(len(a[i])):
            ...
            tmp.append(float(d[i][j]))
            tmp.append(float(e[i][j]))
...
def union_accuracy_average(a,b, c, d, e, X_test, y_test):
    unionnn = []
    for i in range(len(X_test)):
        unionnn.append([])
        for j in range(len(a[i])):
...
def union_accuracy_max(a,b, c, d, e, X_test, y_test):
    unionnn = []
    for i in range(len(X_test)):
        unionnn.append([])
        for j in range(len(a[i])):

            unionnn[i].append( max(float(a[i][j]),float(b[i][j]),float(c[i][j]), float(d[i][j]), float(e[i][j]) ) )
            #unionnn[i].append(float(a[i][j]) * float(b[i][j]) * float(c[i][j]) * float(d[i][j]))
...
def union_accuracy_multiply(a,b, c, d, e, X_test, y_test):
    unionnn = []
    for i in range(len(X_test)):
        unionnn.append([])
        for j in range(len(a[i])):
            ...
            unionnn[i].append(float(a[i][j]) * float(b[i][j]) * float(c[i][j]) * float(d[i][j]) * float(e[i][j]))
...
def union_accuracy_weighted_median(a,b, c, m, n, X_test, y_test, e, f, g, p, r):
    unionnn = []
    for i in range(len(X_test)):
        unionnn.append([])
        for j in range(len(a[i])):
            tmp = []
            tmp.append(float(a[i][j])*e[j])
            tmp.append(float(b[i][j]) * f[j])
            tmp.append(float(c[i][j]) * g[j])
            tmp.append(float(m[i][j]) * p[j])
            tmp.append(float(n[i][j]) * r[j])
            tmp_median = statistics.median(tmp)
            unionnn[i].append(tmp_median)
...
def union_accuracy_weighted_average(a,b, c, m, n, X_test, y_test, e, f, g, p, r):
    unionnn = []
    for i in range(len(X_test)):
        unionnn.append([])
        for j in range(len(a[i])):
...
union_accuracy_median(clf8_fitted_model.predict_proba(X_test), clf4_fitted_model.predict_proba(X_test), clf12_fitted_model.predict_proba(X_test), clf18_fitted_model.predict_proba(X_test), clf11_fitted_model.predict_proba(X_test), X_test, y_test)

union_accuracy_average(clf8_fitted_model.predict_proba(X_test), clf4_fitted_model.predict_proba(X_test), clf12_fitted_model.predict_proba(X_test), clf18_fitted_model.predict_proba(X_test), clf11_fitted_model.predict_proba(X_test), X_test, y_test)

union_accuracy_max(clf8_fitted_model.predict_proba(X_test), clf4_fitted_model.predict_proba(X_test), clf12_fitted_model.predict_proba(X_test), clf18_fitted_model.predict_proba(X_test), clf11_fitted_model.predict_proba(X_test), X_test, y_test)

union_accuracy_multiply(clf8_fitted_model.predict_proba(X_test), clf4_fitted_model.predict_proba(X_test), clf12_fitted_model.predict_proba(X_test), clf18_fitted_model.predict_proba(X_test), clf11_fitted_model.predict_proba(X_test), X_test, y_test)


union_accuracy_weighted_median(clf8_fitted_model.predict_proba(X_test), clf4_fitted_model.predict_proba(X_test), clf12_fitted_model.predict_proba(X_test), clf18_fitted_model.predict_proba(X_test), clf11_fitted_model.predict_proba(X_test), X_test, y_test, clf8_precision, clf4_precision,  clf12_precision, clf18_precision, clf11_precision)

union_accuracy_weighted_average(clf8_fitted_model.predict_proba(X_test), clf4_fitted_model.predict_proba(X_test), clf12_fitted_model.predict_proba(X_test), clf18_fitted_model.predict_proba(X_test), clf11_fitted_model.predict_proba(X_test), X_test, y_test, clf8_precision, clf4_precision,  clf12_precision, clf18_precision, clf11_precision)
